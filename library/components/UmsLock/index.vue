<script setup>
    ...
    formRef.value.validate(async (valid) => {
      if (valid) {
        ...
        setTimeout(async () => {
          ...
          if (document.getElementsByClassName('yes-side-bar')[0])
            document.getElementsByClassName('yes-side-bar')[0].style.display =
              ''
        }, 500)
      }
    })
  ...
  const handleLock = () => {
    _handleLock()
    if (document.getElementsByClassName('yes-side-bar')[0])
      document.getElementsByClassName('yes-side-bar')[0].style.display = 'none'
  }
</script>

<template>
  <yes-icon v-if="theme.showLock" icon="lock-line" @click="handleLock" />
  <transition v-if="theme.showLock" mode="out-in" name="fade-transform">
    <div v-if="lock" class="yes-screen-lock">
      <div
        class="yes-screen-lock-background"
        :style="{
      ...
      <div class="yes-screen-lock-content">
        <div class="yes-screen-lock-content-title">
          <el-avatar :size="180" :src="avatar" />
          <yes-icon :icon="lockIcon ? 'lock-line' : 'lock-unlock-line'" />
          {{ title }} 屏幕已锁定
        </div>
        <div class="yes-screen-lock-content-form">
