...
<template>
  <el-scrollbar
    class="yes-column-bar-container"
    :class="{
      'is-collapse': collapse,
      ['yes-column-bar-container-' + theme.columnStyle]: true,
    }"
  >
    <ums-logo />
    <el-tabs v-model="tab.data" tab-position="left" @tab-click="handleTabClick">
      <template v-for="(item, index) in routes" :key="index + item.name">
        <el-tab-pane :name="item.name">
          <template #label>
            <div
              class="yes-column-grid"
              :class="{
                ['yes-column-grid-' + theme.columnStyle]: true,
              }"
    ...
    >
      ...
      <template v-for="item in partialRoutes" :key="item.path">
        <ums-menu v-if="item.meta && !item.meta.hidden" :item="item" />
      </template>
    </el-menu>
  </el-scrollbar>
</template>
