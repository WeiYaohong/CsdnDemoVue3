  ...
  actions: {
    ...
    afterLogin(token, tokenName) {
      ...
      if (token) {
        ...
        gp.$baseMessage(err, 'error', 'yes-hey-message-error')
        throw err
      }
    },
    /**
    ...
    async login(userInfo) {
      console.log('login')
      console.log('login', tokenName)
      let res = await login(userInfo)
      if (!res.data.success) {
        let msg = `登录接口异常，未正确返回${tokenName}...`
        if (res.msg) {
          msg = res.msg
        }
        gp.$baseMessage(msg, 'error', 'yes-hey-message-error')
        return
      }
      // var token =
      //   'admin-token-25d4D7EA-E7E2-bBeF-25BF-1bFAE3399f18-1646637417837'
      this.afterLogin(res.data.data.token, 'Authorization')
    },
    /**
     * @description 第三方登录
     * @param {*} tokenData
    ...
    async getUserInfo() {
      ...
      let roles = ['Admin']
      let permissions = ['read:system', 'write:system', 'delete:system']
